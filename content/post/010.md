...
假设 name: `odrive` 挂载目录是 `/onedrive` 运行以下命令:
...
```bash
command=" mount odrive:/ /onedrive  --copy-links --no-gzip-encoding --no-check-certificate --allow-other --allow-non-empty --umask 000"
```
...
```
  ...
  现在就可以使用`systemctl`来挂载OneDrive了
